...
def handle_login(email, password):
    try:
        user = UserPlayer.objects.get(email=email)
...
def handle_registration(username, email, password):
    ...
    if UserPlayer.objects.filter(username=username).exists():
        return False, 'Имя уже используется.'
    if len(username) < 4 or not username.strip():
        return False, 'Слишком короткое имя'

    hashed_password = make_password(password)
    user = UserPlayer(username=username, email=email, password=hashed_password)
    user.save()
    return True, user
